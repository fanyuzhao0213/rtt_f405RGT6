...
/* 写传感器寄存器 */
rt_err_t oled_write_reg(struct rt_i2c_bus_device *bus, rt_uint8_t reg, rt_uint8_t data)
{
    rt_uint8_t buf[2];
...
{
	rt_err_t ret =0;
	ret = oled_write_reg(I2C1_BUS,Command_ADDR,Command);
//	rt_kprintf("oled_write_reg ret : %d\n",ret);
}
...
void OLED_WriteData(uint8_t *Data, uint8_t Count)
{
	...
	for (i = 0; i < Count; i ++)
	{
		oled_write_reg(I2C1_BUS,Data_ADDR,Data[i]);
	}
